...
class Game:
    ...
    def get_account_details(self, cookie, ltuid):
        """Retrieve account details - FIXED to use original working API"""
        try:
            ...
            # Find the account for this game
            account_data = accounts[0]  # Use first account
            return {
                "uid": account_data.get("game_role_id", ltuid),
                "nickname": account_data.get("nickname", "Unknown"),
                "rank": account_data.get("level", 0),
                "region": account_data.get("region_name", account_data.get("region_name", "Unknown"))
            }
